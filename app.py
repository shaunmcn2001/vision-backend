# app.py  –  LAWD Parcel Toolkit (private container version)
...
# ─── upload vector → Azure (token kept in URL for private read) ─────
def upload_vector_blob(uploaded_file):
    ...
    # private container → include SAS in manifest URL
    url = f"{bc.url}?{sas}" if sas else bc.url
    return url, uid
...
def fetch(ids):
    ...
    for lp in ids:
        ...
        try:
            js = requests.get(url, params={"where":f"{fld}='{lp}'","returnGeometry":"true","f":"geojson"},
                              timeout=12).json()
...
if static_cfg["basemaps"]:
    bb = next(b for b in static_cfg["basemaps"]
...
if "geoms" in st.session_state:
    ...
    for lp,g in st.session_state["geoms"].items():
        folium.GeoJson(mapping(g),
                       style_function=sty,
                       name=lp).add_child(folium.Popup(lp)).add_to(pg)
        ...
        if st.sidebar.button("💾 Generate KML"):
            ...
            for lp, g in gs.items():
                ...
                for i, poly in enumerate(polys, 1):
                    area = abs(geod.geometry_area_perimeter(poly)[0])/1e4
                    nm = f"{lp} ({i})" if len(polys) > 1 else lp
                    desc = f"Lot/Plan: {lp}<br>Area: {area:,.2f} ha"
                    p = folder.newpolygon(name=nm, description=desc,
                                          outerboundaryis=poly.exterior.coords)
                    for ring in poly.interiors: p.innerboundaryis.append(ring.coords)
