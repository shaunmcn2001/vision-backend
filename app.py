# app.py  –  LAWD Parcel Toolkit  (basemaps + server DBs + dynamic uploads)
...
def show_overlay(msg: str, pct: int | None):
    ...
    </style><div class='ov'><div class='bar'><div></div></div><div class='txt'>{msg}</div></div>"""
    if not ph:
        ph = st.empty()
        st.session_state["_overlay_ph"] = ph
    ph.markdown(html, unsafe_allow_html=True)

# ───────────── vector upload helper (Azure) ────────────
...
st.session_state.setdefault("basemap", static_cfg["basemaps"][0]["name"]
                            if static_cfg["basemaps"] else None)
...
def fetch_geoms(ids):
    ...
    for lp in ids:
        ...
        try:
            ...
            for ft in feats:
                g=shape(ft["geometry"]); grouped[lp].append(transform(tfm,g) if tfm else g)
...
if tab=="Layers":
    ...
    b_names=[b["name"] for b in static_cfg["basemaps"]]
    if b_names:
    ...
    with st.sidebar.expander("➕ Add server layer"):
        ...
        new_layers = st.text_input("Layers (WMS only)")
            ...
            if new_name and new_url:
                static_cfg["databases"].append(
                    {"name":new_name,"type":new_type,"url":new_url,
                     "layers":new_layers,"attr":new_attr})
    ...
    for d in dynamic_cfg:
        cur=st.session_state["dyn_state"].get(d["id"],False)
    ...
    if st.sidebar.button("Save layer") and up_file and up_name:
        try:
            ...
            st.sidebar.success("Layer uploaded.")
            st.experimental_rerun()
    ...
    # manage uploads
    st.sidebar.markdown("### 🗂 Manage uploads")
    for d in dynamic_cfg:
        c1,c2=st.sidebar.columns([5,1])
        with c1:
            st.text_input("", d["name"], key=f"name_{d['id']}")
        with c2:
            if st.button("🗑", key=f"del_{d['id']}"):
                try: BlobClient.from_blob_url(d["url"]).delete_blob()
                except Exception: pass
                dynamic_cfg[:] = [x for x in dynamic_cfg if x["id"]!=d["id"]]
                save_dynamic(dynamic_cfg)
                st.experimental_rerun()
    if st.sidebar.button("Save name changes"):
        for d in dynamic_cfg:
            d["name"]=st.session_state[f"name_{d['id']}"]
        save_dynamic(dynamic_cfg)
        st.sidebar.success("Names updated.")
        st.experimental_rerun()
