#!/usr/bin/env python3
# LAWD Parcel Toolkit · 2025-07-12  (drop-down edition)

"""
Adds a left-click “⋮” button in every AgGrid row.
Click → little drop-down with:
    • Zoom to result
    • Download KML
    • Remove

Those actions post `{type: ..., row: ...}` back to Streamlit via
`window.postMessage`, and Python handles them exactly like a normal
selection—so the rest of the app stays unchanged.
"""

# ——— stdlib ————————————————————————————————————————————
import io, pathlib, requests, tempfile, zipfile
# ——— streamlit & friends ————————————————————————————
...
def kml_colour(hexrgb,pct):
    r,g,b=hexrgb[1:3],hexrgb[3:5],hexrgb[5:7]; a=int(round(255*pct/100))
...
if tab=="Query":
    ...
    if st.sidebar.button("🔍 Search",use_container_width=True) and ids_txt.strip():
        ...
        with st.spinner("Fetching parcels…"): recs,miss=fetch_parcels(ids)
...
if "parcels" in st.session_state:
    ...
    def sty(feat): lp=feat.get("properties",{}).get("name","")
    def sty(feat):
        lp=feat.get("properties",{}).get("name","")
        return {"fillColor":s["fill"],"color":"red" if lp in sel_ids else s["line"],
    ...
    for lp,rec in st.session_state["parcels"].items():
        geom=rec["geom"]; props=rec["props"]
        html=(f"<b>Lot/Plan:</b> {lp}<br>"
              f"<b>Lot Type:</b> {props.get('lottype') or props.get('PURPOSE') or 'n/a'}<br>"
              f"<b>Area:</b> {abs(geod.geometry_area_perimeter(geom)[0])/1e4:,.2f} ha")
        feat={"type":"Feature","properties":{"name":lp},"geometry":mapping(geom)}
        folium.GeoJson(feat,name=lp,style_function=sty,tooltip=lp,popup=html).add_to(pg)
...
if "table" in st.session_state and not st.session_state["table"].empty:
    ...
    class ActionCell {
      init(params) {
        ...
        this.eGui.innerHTML = `<span style="cursor:pointer;font-weight:bold">⋮</span>
        ...
        this.menu = this.eGui.querySelector('.dd');
        this.eGui.onclick = e => {
          if (e.target.dataset.act){
            window.postMessage({type:e.target.dataset.act, row:this.params.data});
    ...
    gob.configure_grid_options(frameworkComponents={"ActionCell":"ActionCell"},
    ...
    # ---- normalize selection ----
    raw_sel = grid.get("selected_rows", None) if isinstance(grid, dict) else getattr(grid, "selected_rows", None)
    ...
    if js and js.get("type")=="zoom":
        ...
        if ids:
            ...
            st.download_button("Download Selected KML", io.BytesIO(kml.kml().encode()),
                               "selected.kml","application/vnd.google-earth.kml+xml")

    elif js and js.get("type")=="remove":
