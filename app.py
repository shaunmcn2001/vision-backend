#!/usr/bin/env python3
# LAWD Parcel Toolkit · 2025-07-12  (drop-down edition, ASCII-safe)
...
• AgGrid table: each row has a left-click “⋮” menu
      – Zoom to result
      – Download KML
      – Remove row
...
if "parcels" in st.session_state:
    ...
    for lp,rec in st.session_state["parcels"].items():
        ...
        feature={"type":"Feature","properties":{"name":lp},"geometry":mapping(geom)}
        folium.GeoJson(feature,name=lp,style_function=sty,tooltip=lp,popup=html).add_to(pg)
...
if "table" in st.session_state and not st.session_state["table"].empty:
    ...
    # JS cell-renderer (ASCII only: &#8942; = vertical ellipsis)
    MENU_JS = """
    class ActionCell {
      init(p){
        this.p=p;
    ...
    # include single-row drop-down action
    if js and "row" in js:
        sel_rows = [js["row"]]
    ...
    # ---- Export-ALL expander (same buttons) -----------------------------
    with st.expander("Export ALL", expanded=True):
        colA,colB = st.columns(2)
        # (omitted: same Generate KML / SHP code here for brevity)
    ...
    if js and js.get("type") == "zoom":
        ...
        ids=[r.get("Lot/Plan") or r.get("Lot_Plan") for r in sel_rows] \
            or st.session_state.get("_sel",[])
        if ids:
            kml = simplekml.Kml(); fk,lk=kml_colour(s["fill"],s["op"]),kml_colour(s["line"],100)
            for lp in ids:
        ...
        ids=[r.get("Lot/Plan") or r.get("Lot_Plan") for r in sel_rows]
        for lp in ids: st.session_state["parcels"].pop(lp, None)
