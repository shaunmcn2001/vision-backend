#!/usr/bin/env python3
# LAWD Parcel Toolkit · 2025-07-12  (stable button-menu build)

"""
• Query QLD / NSW cadastre by Lot-Plan IDs
• Folium map with coloured parcel layer
• AgGrid table:
      – Checkbox multi-select
      – “⋮” button in each row opens a pop-up (Zoom · Download KML · Remove)
...
# ─── STATIC CONFIG (basemaps / overlays) ───
...
class ActionCell {
  init(p){
    ...
    btn.innerHTML='&#8942;';
    btn.style.cssText='cursor:pointer;border:none;background:none;font-weight:bold';
    const menu=document.createElement('div');
    menu.style.cssText='display:none;position:fixed;background:#fff;border:1px solid #ccc;'
      +'border-radius:4px;min-width:120px;box-shadow:0 2px 6px rgba(0,0,0,.15);'
      +'font-size:12px;z-index:10000';
    menu.innerHTML='<div class="it" data-act="zoom">Zoom to result</div>'
...
""")
    ...
    with st.expander("Export ALL", expanded=True):
        ...
        if c2.button("Generate SHP"):
            ...
            gpd.GeoDataFrame(st.session_state["table"],
                geometry=[r["geom"] for r in st.session_state["parcels"].values()],
                crs=4326).to_file(tmp+"/parcels.shp")
            z=pathlib.Path(tmp,"parcels.zip")
            with zipfile.ZipFile(z,"w",zipfile.ZIP_DEFLATED) as zf:
                for f in pathlib.Path(tmp).glob("parcels.*"): zf.write(f,f.name)
            st.download_button("Download SHP", open(z,"rb"),
                               "parcels.zip","application/zip")

    # ---- per-row actions ---------------------------------------------
    if js and js.get("type") in {"zoom","kml","remove"} and "row" in js:
        row = js["row"]
        lp  = row.get("Lot/Plan") or row.get("Lot_Plan")
        if lp:
            if js["type"]=="zoom":
                ...
                poly.style.polystyle.color=fk; poly.style.linestyle.color=lk; poly.style.linestyle.width=float(s["w"])
                st.download_button("Download selected KML", io.BytesIO(poly.kml().encode()),
