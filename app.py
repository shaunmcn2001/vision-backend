...
# ───────── STATIC CONFIG ───────────────────────────────#!/usr/bin/env python3
# LAWD Parcel Toolkit · 2025-07-12
...
if page == "Query":
    ...
    with col_ctrl:
        ...
        with st.expander("⚙️ Style & KML", expanded=False):
            ...
            lw = st.slider("Width px", 0.5, 6.0, 1.2, 0.1)
    ...
    with col_map:
        # Build & render full-screen map
        m = folium.Map(location=[-25, 145], zoom_start=5,
                       control_scale=True, width="100vw", height="100vh")
        ...
        result = st_folium(m, key="map")
        if result and result.get("bounds"):
            st.session_state["last_bounds"] = result["bounds"]
...
if page == "Query":
    ...
    with col_map:
        ...
        # **Call without** returned_bounds — get full data dict
        result = st_folium(m, key="map")
        # save latest bounds if we need them
        if result and result.get("bounds"):
            st.session_state["last_bounds"] = result["bounds"]

# ───────── LAYERS & DOWNLOADS tabs unchanged ───────────
if page == "Layers":
    st.sidebar.header("Basemap")
    if cfg["basemaps"]:
        names = [b["name"] for b in cfg["basemaps"]]
